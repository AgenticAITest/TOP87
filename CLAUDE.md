...
- **Deployment** is containerized: a multi-stage `Dockerfile` builds the static bundle (`npm run build`) and serves `dist/` via nginx (`deploy/nginx.conf`); `docker-compose.prod.yml` wires it behind a shared external Traefik network (`proxy`) on `top87.id` (www→apex redirect, Let's Encrypt TLS). `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` are passed as build args (baked in at build time). There is no CI — deploys are run manually on the VPS. The older manual `scp` static deploy path is recorded in project memory.
