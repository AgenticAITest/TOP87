    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <h2 className="font-serif text-3xl font-bold text-white mb-3">Submitted!</h2>
          <p className="text-gray-500 text-sm mb-8 max-w-xs mx-auto">
            Your media has been sent to the charter admin for review.
          </p>
          <button onClick={reset}
            className="glass-gold px-6 py-2.5 rounded-full text-sm font-bold uppercase tracking-widest text-gold hover:bg-gold hover:text-charcoal transition-all">
            Submit Another
          </button>
        </motion.div>
      </div>
  ...
  return (
    <div className="min-h-screen py-32 px-6">
      <div className="max-w-2xl mx-auto">
        <span className="text-xs uppercase tracking-[0.3em] text-gold/60 mb-3 block">Gallery</span>
        <h1 className="font-serif text-4xl font-bold text-white mb-2">Submit Media</h1>
          ...
          {tabs.map(t => (
            <button key={t.id} type="button" onClick={() => handleTypeChange(t.id)}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest transition-all ${
                type === t.id ? 'bg-gold/15 text-gold border border-gold/30' : 'glass text-gray-400 hover:text-white'
              }`}>
              {t.icon}
              {t.label}
            </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              {preview ? (
                ...
                <div onClick={() => fileRef.current?.click()}
                  className="w-full h-44 rounded-2xl border-2 border-dashed border-white/10 hover:border-gold/30 flex flex-col items-center justify-center gap-3 cursor-pointer transition-all group">
                  <Upload size={24} className="text-gray-600 group-hover:text-gold/50 transition-colors" />
                  <p className="text-xs text-gray-600 group-hover:text-gray-400 transition-colors">
                    Click to select a photo <span className="text-gray-700">(max {MAX_PHOTO_MB} MB)</span>
                  </p>
                </div>
              )}
            ...
            <div>
              ...
              {file ? (
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4 flex items-center gap-4">
                  ...
                  <button type="button" onClick={clearFile}
                    className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-colors shrink-0">
                    <X size={14} />
                  </button>
                </div>
              ) : (
                <div onClick={() => fileRef.current?.click()}
                  className="w-full h-44 rounded-2xl border-2 border-dashed border-white/10 hover:border-gold/30 flex flex-col items-center justify-center gap-3 cursor-pointer transition-all group">
                  <Film size={24} className="text-gray-600 group-hover:text-gold/50 transition-colors" />
                  <p className="text-xs text-gray-600 group-hover:text-gray-400 transition-colors">
                    Click to select a video <span className="text-gray-700">(MP4, MOV, MKV — max {MAX_VIDEO_MB} MB)</span>
                  </p>
                </div>
              )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select value={effectiveCharter} onChange={e => setCharterId(e.target.value)} required
                ...
                <option value="" className="bg-zinc-900 text-white">Select charter…</option>
                {charters.map(c => <option key={c.id} value={c.id} className="bg-zinc-900 text-white">{c.name}</option>)}
              </select>
            </div>
            <div>
              ...
              <select value={yearTaken} onChange={e => setYearTaken(e.target.value)}
                className="w-full bg-zinc-900 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-gold/50 transition-colors">
                <option value="" className="bg-zinc-900 text-white">Select year…</option>
          ...
          <button type="submit" disabled={submitMutation.isPending}
            className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gold hover:bg-gold/90 text-charcoal font-bold py-3 px-8 rounded-full transition-all disabled:opacity-50 uppercase tracking-widest text-sm">
